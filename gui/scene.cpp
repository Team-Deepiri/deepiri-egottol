...
QList<WireItem*> SchematicScene::wires() const {
    return d->wires_;
}
...
double SchematicScene::grid_size() const {
    return d->grid_size_;
}

void SchematicScene::set_snap_to_grid(bool snap) {
    d->snap_to_grid_ = snap;
}

bool SchematicScene::snap_to_grid() const {
    return d->snap_to_grid_;
}
...
void SchematicScene::set_wire_tool(WireTool* tool) {
    d->wire_tool_ = tool;
}
