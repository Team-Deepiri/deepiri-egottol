...
WireTool::WireTool(QObject *parent) : QObject(parent), d(new WireToolImpl) {
  d->wire_color_ = Qt::green;
}

WireTool::~WireTool() { delete d; }
...
bool WireTool::is_active() const { return d->active_; }

void WireTool::set_scene(SchematicScene *scene) { d->scene_ = scene; }

SchematicScene *WireTool::scene() const { return d->scene_; }

void WireTool::handle_click(QGraphicsSceneMouseEvent *event) {
  if (!d->active_ || !d->scene_)
    return;
...
void WireTool::set_wire_color(const QColor &color) { d->wire_color_ = color; }

QColor WireTool::wire_color() const { return d->wire_color_; }
