...
SelectionTool::SelectionTool(QObject *parent)
    : QObject(parent), d(new SelectionToolImpl) {}

SelectionTool::~SelectionTool() { delete d; }
...
bool SelectionTool::is_active() const { return d->active_; }

void SelectionTool::set_scene(SchematicScene *scene) { d->scene_ = scene; }

SchematicScene *SelectionTool::scene() const { return d->scene_; }
...
ComponentItem *
SelectionTool::find_component_underMouse(const QPointF &pos) const {
  if (!d->scene_)
    return nullptr;
