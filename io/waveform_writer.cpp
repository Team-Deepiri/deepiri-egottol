...
bool WaveformWriter::write(const std::string& filename, const std::vector<WaveformData>& waveforms) {
...
bool WaveformWriter::writeCSV(const std::string& filename, const std::vector<WaveformData>& waveforms) {
    ...
    for (size_t i = 0; i < max_size; ++i) {
        if (i < waveforms[0].time_points.size()) {
            file << std::scientific << std::setprecision(6) << waveforms[0].time_points[i];
        } else {
            file << "0";
        }
...
bool WaveformWriter::writeVCD(const std::string& filename, const std::vector<WaveformData>& waveforms) {
    std::ofstream file(filename);
    if (!file.is_open()) {
        return false;
    }
    ...
    char current_id = '!';
    for (size_t i = 0; i < max_size; ++i) {
        double current_time = (i < waveforms[0].time_points.size()) ? waveforms[0].time_points[i] : 0.0;
...
WaveformFormat WaveformWriter::getFormat() const {
    return pImpl->format_;
}

std::string WaveformWriter::toString(const std::vector<WaveformData>& waveforms) {
