...
const std::vector<double>* SimulationData::getSignal(const std::string& name) const {
    auto it = pImpl->signals.find(name);
    if (it != pImpl->signals.end()) return &it->second;
    return nullptr;
}

std::vector<std::string> SimulationData::getSignalNames() const {
    std::vector<std::string> names;
    for (const auto& s : pImpl->signals) names.push_back(s.first);
    return names;
}
